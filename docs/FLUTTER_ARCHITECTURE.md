...
Surrounding these: contributions/invoices, charity drives, group shopping, standalone grocery plans, a trust layer (guarantors, KYC, ratings, reputation), in-group chat & disputes, wallet/payouts/bank accounts, notifications, discovery/search/insights, gamification/leaderboard, a friends/invites social graph, profile/settings, legal/consent, a school-fee module, and an administration layer.
...
| **Engagement** | Streaks, badges, Keens coin, leaderboard | `gamification/*` |
| **Social & growth** | Friendships (request/accept), invites, referrals | `friends*` (web), `referrals*` |
| **Compliance** | Terms, Privacy, consent, account deletion | `terms`/`privacy` (web), *(deletion: gap)* |
...
### Feature: `gamification` (gamification + leaderboard)
- **Business purpose:** Drive retention through motivation.
- **Responsibilities:** Streaks/freezes, badges, Keens balance surfacing, leaderboard.
- **Screens:** Leaderboard, Badges/achievements (ui-designs **N**: `leaderboard_rankings`, `my_achievements_badges`).
- **Dependencies:** `session`, `core`, `design_system`.
- **Justification:** A cohesive *engagement* capability (read-mostly, flag-gated, motivational). **Referrals were removed from here** and merged into `friends` (below), mirroring the codebase change where `/referrals` redirects to `/friends` — keep gamification focused on streaks/badges/leaderboard.

### Feature: `friends` (social graph + invites/referrals)
- **Business purpose:** Grow the network through real social connections and invites.
- **Responsibilities:** Mutual friend graph (request → accept/decline/cancel/remove), and the invite/referral block (invite link + code + WhatsApp share + qualify/reward tracking). One screen serves both, matching the web's merged `/friends` page (the old `/referrals` route redirects here).
- **Screens:** Friends & Invites (single screen: add-friend, incoming requests, friends list, outgoing pending, invite/referral block). **Design gap:** only `refer_earn` (ui-designs **N**) — the referral block — is rendered; the friend-graph sections are undesigned (see UI_DESIGN_FEATURE_MAP §5).
- **Workflows:** Add by phone/email/username → recipient notified → accept (auto-accept if reciprocal) / decline / cancel / remove. Invite link share → referral qualifies on register/first-join (`REFERRAL_QUALIFY_ON`); rewards only if `REFERRAL_REWARD_ENABLED`.
- **Dependencies:** `session`, `notifications` (Friend Request/Accepted), `core`, `design_system`, share/deep-link util.
- **Shared resources:** referral-code/invite-link service; the directory user-lookup (by phone/email/username).
- **Justification:** A distinct **social** domain (`Friendship` entity, request lifecycle) that the codebase added as its own capability. It is not gamification (no streaks/badges) and not profile (it's relationship management, not identity presentation). Merging invites here matches the product: friends and "invite people to KeenPocket" are the same growth surface. **Backend: friend graph is web-only today; referral tracking is live.**

### Feature: `legal` (Terms, Privacy & consent)
- **Business purpose:** Compliance, trust, and store-review requirements.
- **Responsibilities:** Render static Terms of Service & Privacy Policy (bundled in-app for offline + remotely refreshable), surface consent at signup, and host the **account-deletion / data-export** entry (store requirement).
- **Screens:** Terms of Service, Privacy Policy, Delete-account confirm (MOBILE_STITCH_PROMPT **Phase R**). **Design gap:** undesigned.
- **Dependencies:** `session` (signed-in delete path), `core`, `design_system`. Linked from `auth` (signup consent) and `profile` (Settings).
- **Justification:** Small but cross-cutting and legally load-bearing; isolating it keeps the legal text, consent capture, and deletion policy in one auditable place rather than scattered across auth/settings. **Backend gap: account-deletion/export endpoints do not exist yet.**

### Feature: `profile` (profile + settings)
- **Business purpose:** Identity presentation and account management.
- **Responsibilities:** Own profile (identity + reputation + badges + ratings), public profile, settings (avatar, notification prefs, password, bank accounts entry, dark mode, logout, links to Legal).
- **Screens:** My profile, Public profile, Settings (ui-designs **O**).
- **Dependencies:** `trust` (reputation/ratings/KYC), `money` (bank accounts), `gamification` (badges), `legal` (Terms/Privacy/delete-account links), `session`, `core`, `design_system`.
...
└── features/
    ...
    ├── discovery/  gamification/  friends/  profile/  legal/
...
|---|---|---|
| `auth` / `session` | `register`, `login`, `logout`, `refresh-token`, `request-token`, `verify-token`, `otp/*`, `change-password` (email password-reset is a web/broker flow) | ✅ live; **mobile password-reset API: gap** |
...
| `gamification` | `gamification/me`, `users/{id}/badges` | ✅ live |
| `friends` | `referrals`, `referrals/me` (invite/referral tracking) | ⚠️ referral tracking live; **friend-graph API: gap — web-only** |
| `legal` | `terms`, `privacy` (web views); account-deletion/export | ❌ **gap — web-only; deletion endpoint missing** |
