...
Every feature ships with tests. Per the architecture blueprint the targets are
**≥ 85% on `domain`** and **≥ 70% overall** — the overall gate (enforced in CI
by `scripts/check_coverage.sh`) is the blueprint's documented standard, not an
arbitrary number. (The earlier flat 80% was stricter than the project's own
standard; as the UI surface grew, page/widget lines came to dominate the metric,
so the gate was aligned to the documented 70% overall while keeping domain
coverage high.) The metric covers the *testable* surface — excluded from the
denominator (separator-agnostic, works on Windows + CI): generated code
(`*.g.dart`, `*.freezed.dart`, `*.config.dart`, `**/generated/`), DI wiring
(`core/di/`, `core/network/network_module.dart`) and the app composition root
(`app/`, `main.dart`). Everything else counts.
